---
title: '一条告警反复发了五天，根因藏在 18 秒的时间差里'
description: '服务发现数据残留告警连发五天，顺着三个服务的日志挖出五环 Bug 链：18 秒的时序窗口、被误杀的删除事件、只删一半的人工操作，附反证法排查技巧'
pubDate: 2026-07-08
category: incident
tags: ['排查思路', '服务发现', 'Kubernetes', '缓存', '线程池', '事故复盘', 'Java']
---

# 一条告警反复发了五天，根因藏在 18 秒的时间差里

> **TL;DR** 一条数据不一致的告警连发五天：某个 IP 的 pod 在 k8s 里早就死了，数据库里却一直留着它的记录，怎么都清不掉。我顺着三个服务的日志往回挖，最后发现这不是一个 bug，而是**五个问题手拉手**才凑出来的事故：起点是 pod 启动和用户配置之间 18 秒的时间差，中间是一次被内存缓存"误杀"的删除操作，结尾还有一次只删了一半的人工操作。这篇文章把排查过程完整摊开，顺便分享一个很好用的排查技巧——反证法。
...
告警从哪天开始的？带着「集群名 + IP」往前逐天扫，扫到 7 月 2 日，找到了第一条相关记录。但诡异的是——**方向和告警是反的**：
...
> 这个技巧我叫它**反证法排查**：正向找不到操作日志时，找一个"陪跑样本"（这里是同一批写入的 `.8`），看它的存活状态能排除哪些操作路径。每排除一条，剩下的可能性就浓缩一分，直到只剩一个说得通的答案。
...
这次排查前后翻了两台机器、三个服务、大约 60G 的日志，工具只有 grep。但方法比工具重要：先确认实体身份，再找第一案发现场，沿时间线收敛，正向查不到就上反证法。如果你也在维护类似的服务发现或数据同步链路，希望这条 Bug 链能帮你提前排掉几颗雷。
